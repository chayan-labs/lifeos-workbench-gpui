#!/bin/zsh
# Assemble Workbench.app from a release build (issue #30).
# The .app format is three files and a plist - no bundler tool needed.
#
#   scripts/bundle-app.sh [output-dir]   (default: dist/)
#
# Produces: <out>/Workbench.app, ad-hoc codesigned, plus a zip for CI.
set -euo pipefail

repo="$(cd "$(dirname "$0")/.." && pwd)"
out="${1:-$repo/dist}"
app="$out/Workbench.app"

echo "==> building release binary"
cargo build --release --manifest-path "$repo/workbench/Cargo.toml"
...
(cd "$out" && ditto -c -k --keepParent Workbench.app Workbench.app.zip)

echo "done: $app"
echo "      $out/Workbench.app.zip"
