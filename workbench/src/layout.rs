//! Zellij-model tiling layout engine: a binary split tree per tab, one
//! focused pane per tab. Pure data + geometry - no rendering here, so the
//! whole thing is unit-testable without a terminal.
//!
...
impl LayoutNode {
    ...
    /// Compute each pane's screen rectangle by recursive halving.
                ...
                let (a, b) = match dir {
                    ...
                    SplitDir::Vertical => {
                        ...
                        (
                            ...
                            Rect {
                                y: area.y + h1,
                                height: area.height - h1,
                                ..area
                            },
                        )
                    }
                };
                let mut v = first.rects(a);
                v.extend(second.rects(b));
                v
            }
        }
    }
...
impl Layout {
    ...
    pub fn focus_next(&self) -> Layout {
        ...
        let focused = panes[(pos + 1) % panes.len()];
        self.with_tab(
            Tab {
                root: tab.root.clone(),
                focused,
    ...
    pub fn focus_prev(&self) -> Layout {
        ...
        let panes = tab.root.panes();
        let pos = panes.iter().position(|p| *p == tab.focused).unwrap_or(0);
        let focused = panes[(pos + panes.len() - 1) % panes.len()];
        self.with_tab(
            Tab {
                root: tab.root.clone(),
                focused,
...
mod tests {
    ...
    #[test]
    fn rects_tile_the_full_area_without_overlap() {
        let (layout, _) = Layout::new().split_focused(SplitDir::Horizontal);
        let (layout, _) = layout.split_focused(SplitDir::Vertical);
        let area = Rect::new(0, 0, 80, 24);
        let rects = layout.tab().root.rects(area);
